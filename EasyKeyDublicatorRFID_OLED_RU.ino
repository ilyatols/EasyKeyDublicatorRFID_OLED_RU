/*
  ...
  Переведенная версия на GitHub: 
  Автор: МЕХАТРОН DIY, AlexMalov, 2019
  Переведено и оптимизировано: Sedukh
...
void OLED_printKey(byte buf[8], byte msgType = 0){
  String st;
  switch (msgType){
    case 0: st = String(EEPROM_key_index) + "-sq rk.x bp " + String(EEPROM_key_count) + " rk.xtq"; break;      
    case 1: st = "Pf;vb ryjgre lkz c[hy"; break; 
    case 3: st = "Rk.x e;t d gfvznb!"; break;   
  }
  myOLED.setFont(RusFont);
  myOLED.clrScr();
  if (msgType == 3) {
    myOLED.print(st, 10, 0);
  } else {
    myOLED.print(st, 0, 0);
  }
  st = "";
  ...
  myOLED.print(st.substring(0, st.length()-1), 18, 14);
  switch (keyType){
    ...
    case keyUnknown: st = "Ytbpdtcnysq"; break;
  }
  if (keyType == keyUnknown) {
    myOLED.setFont(RusFont);
  } else {
    myOLED.setFont(SmallFont);
  }
  myOLED.print(st, 32, 24);
  myOLED.update();
}
...
void OLED_printError(String st, bool err = true){
  ...
  myOLED.setFont(RusFont);
  Serial.begin(115200);
  if (!err) Serial.print("success");
    else Serial.print("error");
  myOLED.print(st, CENTER, 16);  
  myOLED.update();
}
...
void setup() {
  ...
  if (EEPROM_key_count == 0 ) {
    myOLED.print(F("Ghjxbnfq rk.x"), CENTER, 7);
  } else {
    myOLED.print(F("Ghjxbnfq rk.x"), CENTER, 16);
  }
  myOLED.setFont(TinyFont);
  myOLED.print("by MEXATPOH", RIGHT, 0);
  myOLED.update();
  ...
  if (EEPROM_key_count != 0 ) {
    ...
    Serial.print(F("Xntybt rk.xf bp gfvznb"));
    ...
    myOLED.setFont(RusFont);
    myOLED.print(F("D gfvznb ytn rk.xtq"), CENTER, 24);
    myOLED.update();  
  }
...
bool write2iBtnTM2004(){                // функция записи на TM2004
  ...
  if (!result){
    ...
    OLED_printError(F("Jib,rf rjgbhjdfybz!"));
    Sd_ErrorBeep();
  ...
  OLED_printError(F("Rk.x crjgbhjdfy"), false);
  Sd_ReadOK();
  delay(2000);
  digitalWrite(R_Led, HIGH);
  return true;
...
bool write2iBtn(){
  ...
  if (Check == 8) {                           // если коды совпадают, ничего писать не нужно
    ...
    OLED_printError(F("Gjlytcty njn ;t rk.x!"));
    Sd_ErrorBeep();
    digitalWrite(R_Led, HIGH);
    delay(1000);
    return false;
...
bool searchIbutton(){
  ...
  if (addr[0] == 0x01) {                              // это ключ формата dallas
    ...
    if (OneWire::crc8(addr, 7) != addr[7]) {
      ...
      OLED_printError(F("CHC ytdthysq!"));
      Sd_ErrorBeep();
...
void loop() {
  ...
  if (echo == 'e'){
    ...
    myOLED.update();
    myOLED.setFont(RusFont);
    myOLED.print(F("Gfvznm jxbotyf!"), CENTER, 7);
    myOLED.print(F("Cxbnfqnt rk.x"), CENTER, 24);
  ...
  if ((copierMode != md_empty) && enc1.isHolded()){     // Если зажать кнопкку - ключ сохранися в EEPROM
    if (EPPROM_AddKey(keyID)) {
      OLED_printError(F("Rk.x ecgtiyj cj[hfyty"), false);
      Sd_ReadOK();
      delay(1000); 
    }
      else Sd_ErrorBeep();
...
void Sd_StartOK(){   // звук "Успешное включение"
  ...
  tone(speakerPin, NOTE_A7); delay(100);
  tone(speakerPin, NOTE_B6); delay(100); 
  tone(speakerPin, NOTE_A6); delay(100);
  noTone(speakerPin); 
}
